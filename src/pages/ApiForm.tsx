    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between p-4 border-b border-slate-700/50">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => navigate('/apis')} className="px-4 py-2 text-slate-400 hover:text-white">
            取消
          </button>
              ...
              <div className="max-w-2xl space-y-6">
                <div className="bg-[#1e293b]/60 rounded-xl border border-slate-700/50 p-6 space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <div className="flex gap-2">
                        ...
                        <button onClick={() => formData.databaseName && loadTables(formData.databaseName)} className="px-3 py-2 bg-slate-700 rounded-lg text-slate-300 hover:bg-slate-600">
                    ...
                    <div>
                      ...
                      <div className="flex gap-4">
                        <label className="flex items-center gap-2 text-white">
                          <input type="radio" name="method" checked={formData.method === 'GET'} onChange={() => setFormData({ ...formData, method: 'GET' })} className="accent-purple-500" />
                          GET
                        </label>
                        <label className="flex items-center gap-2 text-white">
                          <input type="radio" name="method" checked={formData.method === 'POST'} onChange={() => setFormData({ ...formData, method: 'POST' })} className="accent-purple-500" />
                          POST
                        </label>
                      </div>
                    </div>
                  ...
                  <div>
                    ...
                    <input type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} placeholder="例如：用户查询" className="w-full px-4 py-2.5 bg-slate-800/50 border border-slate-700/50 rounded-lg text-white placeholder-slate-500" />
                  </div>
                  <div>
                    <label className="block text-sm text-slate-400 mb-2">API路径 *</label>
                    <input type="text" value={formData.path} onChange={(e) => setFormData({ ...formData, path: e.target.value })} placeholder="例如：/api/user/list" className="w-full px-4 py-2.5 bg-slate-800/50 border border-slate-700/50 rounded-lg text-white placeholder-slate-500" />
                  </div>
                  <div>
                    <label className="block text-sm text-slate-400 mb-2">描述</label>
                    <textarea value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} rows={3} className="w-full px-4 py-2.5 bg-slate-800/50 border border-slate-700/50 rounded-lg text-white placeholder-slate-500" />
                  </div>
                </div>
              </div>
            )}
              ...
              <div className="space-y-6">
                <div className="bg-[#1e293b]/60 rounded-xl border border-slate-700/50 p-6">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <label className="flex items-center gap-2 text-white">
                      <input type="checkbox" checked={formData.mockEnabled === 1} onChange={(e) => setFormData({ ...formData, mockEnabled: e.target.checked ? 1 : 0 })} className="accent-purple-500" />
                      启用Mock
                    </label>
                  </div>
                  {formData.mockEnabled === 1 && (
                    <div className="space-y-4">
                      <button onClick={generateMockData} className="px-3 py-1 bg-purple-500 text-white rounded-lg text-sm">
                        根据字段生成
                      </button>
      ...
      <div className="flex items-center justify-between p-4 border-t border-slate-700/50">
        <button onClick={() => setCurrentStep(Math.max(0, currentStep - 1))} disabled={currentStep === 0} className="flex items-center gap-2 px-4 py-2 text-slate-400 hover:text-white disabled:opacity-50">
          <ArrowLeft className="w-4 h-4" />
          上一步
        </button>
        <button onClick={() => { if (currentStep < STEPS.length - 1) setCurrentStep(currentStep + 1); }} disabled={!canProceed()} className="flex items-center gap-2 px-6 py-2 bg-purple-500 text-white rounded-lg hover:bg-purple-600 disabled:opacity-50">
